...
layui.define(['element', 'common'], function (exports) {
                    ...
                    if ($this.find('dl').length > 0) {
                        var $dd = $this.find('dd').each(function () {
                            $(this).on('click', function () {
                                ...
                                var menuUrl = $a.data('url');
                                //var icon = $a.children('i:first').data('icon');
                                var menuName = $a.children('cite').text();
                                var data = {
                                    elem: $a,
                                    field: {
                                        href: menuUrl,
    ...
    function getHtml(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i].childMenus !== undefined && data[i].childMenus !== null && data[i].childMenus.length > 0) {
                ...
                for (var j = 0; j < data[i].childMenus.length; j++) {
                    ulHtml += '<dd title="' + data[i].childMenus[j].menuName + '">';
                    ulHtml += '<a href="javascript:;" data-url="' + data[i].childMenus[j].menuUrl + '">';
                    
                    ulHtml += '<cite>' + data[i].childMenus[j].menuName + '</cite>';
                    ulHtml += '</a>';
                    ulHtml += '</dd>';
                }
                ulHtml += '</dl>';
            } else {
                var dataUrl = (data[i].menuUrl !== undefined && data[i].menuUrl !== '') ? 'data-url="' + data[i].menuUrl + '"' : '';
